"""
[3단계] DART API로 재무제표를 가져와서 Beneish M-Score를 계산.

실행 전 준비물: fraud_labels_matched.csv (2단계 결과물)
실행하면 -> beneish_scores.csv 파일이 생성됩니다.
"""
...
def compute_beneish_m(t, t1):
    ...
    # DEPI: 감가상각비가 없으면 중립값(1.0)으로 대체 (현금흐름표에만 있는 경우가 많아 결측 잦음)
    try:
    ...
    # TATA: 감가상각비가 없으면 0으로 대체하여 운전자본 변화만 반영 (근사치)
    try:
...
def extract_first_year(violation_years_str):
    m = re.search(r"(20\d{2})", str(violation_years_str))
    return int(m.group(1)) if m else None


def main():
    df = pd.read_csv("fraud_labels_matched.csv", dtype={"corp_code": str})
    ...
    for _, row in df.iterrows():
        ...
        year = extract_first_year(row["violation_years"])

        if year is None:
            print(f"[건너뜀] {corp_name}: 위반연도 특정 불가")
            continue
        ...
        row_result = {
            ...
            "m_score": m_score,
            "flag_high_risk": (m_score is not None and m_score > -1.78),
        }
    ...
    out.to_csv("beneish_scores.csv", index=False, encoding="utf-8-sig")
    print(f"\n완료. {len(out)}건 처리 -> beneish_scores.csv 저장")
