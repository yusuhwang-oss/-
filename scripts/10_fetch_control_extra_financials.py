...
- 03번 스크립트와 계산 로직은 동일하고, 대상만 control_labels_extra.csv로 바뀐 버전입니다.

실행 전 준비물: control_labels_extra.csv (4단계 결과물)
실행하면 -> control_scores_extra.csv 파일이 생성됩니다.
...
def main():
    df = pd.read_csv("control_labels_extra.csv", dtype={"corp_code": str})
    ...
    out.to_csv("control_scores_extra.csv", index=False, encoding="utf-8-sig")
    print(f"\n완료. {len(out)}건 처리 -> control_scores_extra.csv 저장")
